...
class MultiSlidePPTGenerator:
    ...
    async def generate_ppt(
        ...
        try:
            ...
            # 保存大纲数据
            outline_file = ppt_dir / "data" / "outline.json"
            outline_file.write_text(
                json.dumps(outline, ensure_ascii=False, indent=2),
                encoding='utf-8'
            )
            logger.info(f"[MultiSlidePPTGenerator] 大纲数据已保存: {outline_file}")
    ...
    def _create_slides_metadata(
        ...
        metadata = {
            "ppt_title": ppt_config.get('ppt_title', 'Untitled Presentation'),
            "total_slides": total_slides,
            "generation_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "style": ppt_config.get('style', 'business'),
            "theme": ppt_config.get('theme', 'corporate-blue'),
            "color_scheme": ppt_config.get('colors', {
                "primary": "#2E8B57",
                "secondary": "#FF8C00",
                "accent": "#E6E6FA"
            }),
    ...
    def _wrap_slide_html(
        ...
        # 预先生成按钮HTML，避免在 f-string 中使用复杂表达式和反斜杠
        if prev_slide:
            prev_btn_html = f'<button class="nav-btn" onclick="window.location.href=\'{prev_slide}\'"><i class="fas fa-arrow-left"></i> 上一页</button>'
        else:
            prev_btn_html = '<button class="nav-btn" disabled><i class="fas fa-arrow-left"></i> 上一页</button>'

        if next_slide:
            next_btn_html = f'<button class="nav-btn" onclick="window.location.href=\'{next_slide}\'">下一页 <i class="fas fa-arrow-right"></i></button>'
        else:
            next_btn_html = '<button class="nav-btn" disabled>下一页 <i class="fas fa-arrow-right"></i></button>'
