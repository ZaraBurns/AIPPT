...
class DesignCoordinator:
    ...
    def _build_design_prompt(
        ...
        style_hint = self.style_guides.get(style, '根据主题内容自由选择合适的配色')
...
**重要提示**:
- 切勿使用 CSS 渐变（linear-gradient，radial-gradient） ——它们无法转换为 PowerPoint 格式。
            ...
            with open(style_file, 'r', encoding='utf-8') as f:
                ...
                if isinstance(styles_section, dict):
                    for style_name, cfg in styles_section.items():
                        if isinstance(cfg, dict) and "design_coordinator" in cfg:
                            result[style_name] = cfg.get("design_coordinator", "")

                return result
        except Exception as e:
            logger.error(f"[DesignCoordinator] 加载风格指南失败: {e}")
            # 返回一个空的风格指南，或者可以选择返回默认值
            return {}
