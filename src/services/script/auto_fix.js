...
/**
 * 内联元素 Margin/Padding 修复器
 * 处理错误: 内联元素 <span> 存在 margin-right，PPT 不支持。请移除内联元素的 margin。
 */
class InlineElementMarginFixer extends ErrorFixer {
  canFix() {
    // 匹配错误信息: 内联元素 <xxx> 存在 margin/padding
    const match = this.errorMessage.match(
      /内联元素 <(\w+)> 存在 (margin|padding)/i
    );
    if (match) {
      this.tagName = match[1];
      this.styleType = match[2];
      return true;
    }
    return false;
  }
  ...
  fix() {
    // 内联元素列表
    const inlineElements = ["span", "b", "i", "strong", "em", "u"];
    let fixCount = 0;
    ...
    inlineElements.forEach((tagName) => {
      ...
      elements.forEach((element) => {
        ...
        if (element.hasAttribute("style")) {
          ...
          // 移除所有 margin 相关属性
          style = style.replace(
            /margin(-top|-right|-bottom|-left)?:\s*[^;]+;?/gi,
            ""
          );
          // 移除所有 padding 相关属性
          style = style.replace(
            /padding(-top|-right|-bottom|-left)?:\s*[^;]+;?/gi,
            ""
          );
        ...
        // 2. 移除 class 中与 margin/padding 相关的 Tailwind 类
        if (element.hasAttribute("class")) {
          let className = element.getAttribute("class");
          const originalClassName = className;

          // 移除 Tailwind 的 margin 类: m-*, mx-*, my-*, mt-*, mr-*, mb-*, ml-*
          className = className.replace(/\b(m[trblxy]?-\d+|m-auto)\b\s*/g, "");
          // 移除 Tailwind 的 padding 类: p-*, px-*, py-*, pt-*, pr-*, pb-*, pl-*
          className = className.replace(/\b(p[trblxy]?-\d+)\b\s*/g, "");
    ...
    // 3. 处理 style 标签中的内联元素样式定义
    const styleElements = this.document.querySelectorAll("style");
    styleElements.forEach((styleEl) => {
      let cssContent = styleEl.textContent;
      const originalCss = cssContent;

      // 为每个内联元素标签移除 margin/padding
      inlineElements.forEach((tagName) => {
        // 匹配 span { ... }, span.class { ... } 等
        const regex = new RegExp(
          `(${tagName}(?:\\.[\\w-]+)?\\s*\\{[^}]*)(margin|padding)(-[a-z]+)?:\\s*[^;]+;([^}]*\\})`,
          "gi"
        );

        cssContent = cssContent.replace(
          regex,
          (match, before, prop, sub, after) => {
            return before + after; // 移除 margin/padding 行
          }
        );
      });
    ...
    if (fixCount > 0) {
      this.fixed = true;
      this.fixDescription = `移除了 ${fixCount} 处内联元素的 margin/padding 样式`;
    }
 ...
 * @returns {Promise<{success: boolean, content: string|null, fixers: string[]}>}
 */
async function getFixedHtmlContent(htmlPath, errorMessage) {
  // 按优先级尝试各种修复器
  const fixers = [
    new InlineElementMarginFixer(errorMessage, htmlPath),
  ...
  let hasAnyFix = false;
  const appliedFixers = [];
  let currentContent = fs.readFileSync(htmlPath, "utf-8");
  ...
  for (const fixer of fixers) {
    if (fixer.canFix()) {
      ...
      if (fixed) {
        ...
        currentContent = fixer.dom.serialize();

        // 更新后续修复器的DOM
        for (let i = fixers.indexOf(fixer) + 1; i < fixers.length; i++) {
          const nextFixer = fixers[i];
          nextFixer.htmlContent = currentContent;
          nextFixer.dom = new JSDOM(currentContent);
  ...
  if (hasAnyFix) {
    ...
    return { success: true, content: currentContent, fixers: appliedFixers };
  }

  console.log(`  ❌ 未找到适合的修复器`);
  return { success: false, content: null, fixers: [] };
}
